...
// static char buffer[PGSIZE];
...
#ifndef NONE
    np->pages_in_swapfile = curproc->pages_in_swapfile;   //counter same as parent counter
    np->pages_in_memory = curproc->pages_in_memory;   //counter same as parent counter
    np->total_pagedout = 0;   //reset counter
    np->page_faults = 0;   //reset counter

    for(i=0; i<MAX_PSYC_PAGES; i++)  {   //va and age are the same as parent
    ...
    for(i=0; i<MAX_PSYC_PAGES; i++){   //head is the same as parent
      if (curproc->head->va == np->memory_pg_arr[i].va)
          np->head = &np->memory_pg_arr[i];
      if (curproc->tail->va == np->memory_pg_arr[i].va)   //tail is the same as parent
...
//assignment3
    ...
    if(!(curproc->pid==1||curproc->parent->pid==1)){
      if (removeSwapFile(curproc) != 0)
        panic("failed to remove swapfile at exit\n");
    }
  #endif
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(!(p->pid==1 || p->parent->pid==1))      {
        for(int i=0; i<MAX_PSYC_PAGES; i++)        {   //search for 
          if(p->memory_pg_arr[i].va != (char*)-1)            {
            p->memory_pg_arr[i].age = p->memory_pg_arr[i].age>>1;   //shift counter right by one bit
            int tmp = get_reference_bit_2(p->memory_pg_arr[i].va,p) == 1;
            if(tmp == 1)   //if accessed bit is on, turn leftmost bit on
      ...
      #ifdef AQ
      if(!(p->pid == 1 || p->parent->pid == 1))      {
        for(int i=0; i<MAX_PSYC_PAGES; i++)        {   //search for free space in memory
          if(p->memory_pg_arr[i].va != (char*)-1)          {
            int tmp = get_reference_bit_2(p->memory_pg_arr[i].va,p);   //retrieve reference bit
            if(tmp == 1)   //if reference bit is on change age to 1 else to 0
              p->memory_pg_arr[i].age = 1;  
            else
              p->memory_pg_arr[i].age = 0;        
          }
        }
        struct paging_meta_data* curpage = p->head;
        while(curpage != 0)        {   //for all pages in queue
          if(curpage->prev != 0)          {
            if(curpage->prev->age == 0 && curpage->age == 1)            {
              tmp_va = curpage->prev->va;   //switch places with the page preceding it 
...
procdump(void)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    ...
    cprintf("%d  %d  %d  %d  %s",p->pages_in_memory,
                                  p->pages_in_swapfile,
                                  p->page_faults,
                                  p->total_pagedout,
                                  p->name);
...
//assignment3
int get_reference_bit_2(char *va,struct proc* p)
{
  uint accessed;
  pte_t *pte = walkpagedir(p->pgdir, (void*)va, 0);
  if (!pte){
